...
def get_preprocessing_transforms(image_size: int = 224):
    ...
    return transforms.Compose(
        [
            transforms.Resize((image_size, image_size)),  # resize the image to the input size specified in the config
            transforms.ToTensor(),  # convert the image to a tensor and scale the pixel values to [0, 1]
            transforms.Normalize(mean=mean, std=std),  # Center and reduce variance of the data
        ]
    )
